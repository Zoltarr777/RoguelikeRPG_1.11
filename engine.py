...
def play_game(player, entities, game_map, message_log, game_state, con, panel, constants):
	...
	while not libtcod.console_is_window_closed():
		...
		if move and game_state == GameStates.PLAYERS_TURN:
			...
			if not game_map.is_blocked(destination_x, destination_y):
				...
				if target:
					...
					#sound_thread = threading.Thread(target=playsound, args=('attack.mp3'))
					#sound_thread = threading.Thread(playsound('attack.mp3'))
					#sound_thread.start()
		...
		if exit:
			if game_state in (GameStates.SHOW_INVENTORY, GameStates.DROP_INVENTORY, GameStates.DROP_EQUIPMENT, GameStates.CHARACTER_SCREEN, GameStates.HELP_MENU, GameStates.SHOW_EQUIPMENT_INVENTORY):
				game_state = previous_game_state
